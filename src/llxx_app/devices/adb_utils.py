...
class Adb(object):
    def __init__(self):
        self.checkconnect = COMMAND['checkConnect']
        self.thirdpackage = COMMAND['thirdpackage']
        self.curpknm = COMMAND['getcurpknm']
...
class BatteryInfo(Adb):
    def __init__(self):
        Adb.__init__(self)
        self.getbatteryinfo = COMMAND['getBattery']
...
class CpuInfo(Adb):
    def __init__(self):
        Adb.__init__(self)
        self.getcpuinfo = COMMAND['cpuinfo']
...
class MemInfo(Adb):
    def __init__(self):
        Adb.__init__(self)
        self.getmeminfo = COMMAND['meminfo']
...
class BaseInfo(Adb):
    def __init__(self):
        Adb.__init__(self)
        self.getmobilemodel = COMMAND['mobilemodel']
        self.getmobilecpu = COMMAND['mobilecpu']
        self.getmobilemem = COMMAND['mobilemem']
        self.getdisplay = COMMAND['mobiledilplay']
        self.getsystemversion = COMMAND['mobileversion']
        self.getmobileimme = COMMAND['mobileimme']
...
class CommonFunction(Adb):
    def __init__(self):
        Adb.__init__(self)
        self.installCommand = COMMAND['install']
    ...
    def install(self, iscovered, apkaddress):
        ...
        if iscovered == u"true":
            command = COMMAND['install'] + "-r " + apkaddress
            os.system(command.encode('utf-8'))
            rst_data['msg'] = "安装完毕"
            rst_data['status'] = "OK"

        elif iscovered == u"false":
            command = COMMAND['install'] + apkaddress
